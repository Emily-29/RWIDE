...
def augment(imgs=[], size=256, edge_decay=0.0, only_h_flip=False):
    H, W, _ = imgs[0].shape
    Hc, Wc = [size, size]
...
class PairLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        source_img = (
            read_img(os.path.join(self.root_dir, "hazy", img_name), resize=self.resize)
            * 2
            - 1
        )
        target_img = (
            read_img(os.path.join(self.root_dir, "clear", img_name), resize=self.resize)
            * 2
            - 1
        )

        if self.mode == "train":
            [source_img, target_img] = augment(
                [source_img, target_img], self.size, self.edge_decay, self.only_h_flip
            )
