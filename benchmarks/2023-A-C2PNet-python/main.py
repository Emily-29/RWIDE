...
def curriculum_weight(difficulty):
    ...
    weights = [
        (1 + cl_lambda) if difficulty > x else (1 - cl_lambda) for x in diff_list
    ]
...
def clcr_train(train_model, train_loader, test_loader, optim, criterion):
    ...
    for step in range(start_step + 1, opt.steps + 1):
        ...
        x = data[0].to(opt.device)
        y = data[1].to(opt.device)
        # n1 = n1.to(opt.device)
        # n2 = n2.to(opt.device)
        # n3 = n3.to(opt.device)
        # n4 = n4.to(opt.device)
...
if __name__ == "__main__":
    ...
    pytorch_total_params = sum(
        p.nelement() for p in net.parameters() if p.requires_grad
    )
