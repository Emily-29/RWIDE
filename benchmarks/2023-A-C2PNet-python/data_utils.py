...
class RESIDE_Dataset(data.Dataset):
    def __init__(self, path, train, size=crop_size, format=".jpg"):
        ...
        self.train = train
        self.format = format
        self.haze_imgs_dir = os.listdir(os.path.join(path, "hazy_align"))
        self.haze_imgs = [
            os.path.join(path, "hazy_align", img) for img in self.haze_imgs_dir
        ]
        self.clear_dir = os.path.join(path, "clear_align")
    ...
    def __getitem__(self, index):
        ...
        new_width = width // 2
        new_height = height // 2
        haze = haze.resize((new_width, new_height))
        ...
        clear = clear.resize((new_width, new_height))
...
class LMDB_Image:
    ...
    def get_image(self):
        ...
        return (
            haze.reshape(*self.size, self.channels),
            clear.reshape(*self.size, self.channels),
            # n1.reshape(*self.size, self.channels),
            # n2.reshape(*self.size, self.channels),
            # n3.reshape(*self.size, self.channels),
            # n4.reshape(*self.size, self.channels),
            # n5.reshape(*self.size, self.channels),
            # n6.reshape(*self.size, self.channels),
        )
...
class DatasetLMDB(data.Dataset):
    ...
    def aug_data(self, *images):
        ...
        if not isinstance(self.size, str):
            i, j, h, w = tfs.RandomCrop.get_params(
                out[0], output_size=(self.size, self.size)
            )
        ...
        out.append(out[0])
        if opt.norm:
            out[0] = tfs.Normalize(mean=[0.64, 0.6, 0.58], std=[0.14, 0.15, 0.152])(
                out[0]
            )
        return out
        ...
        if isinstance(self.size, int):
            while haze.shape[0] < self.size or haze.shape[1] < self.size:
                ...
                with env.begin() as txn:
                    byteflow = txn.get(self.keys[index])
                IMAGE = pickle.loads(byteflow)
                haze, clear = IMAGE.get_image()

        out = self.aug_data(haze, clear)
        return out
