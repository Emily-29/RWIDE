...
parser.add_argument(
    "--test_dir", type=str, default="/home/sunhang/lzm/deHaze/outdoor_Test/"
)
...
model_path = r"/home/lx/nas/Code/03_RWIDE/01_dehaze-algorithm/02_network/2023-A-PSMBNet-python/PSBDN/output_result/RWIDE-beta/epoch_635_16.39_0.595_outdoor.pkl"

predict_result = SaveFolderPath
...
if args.type == 0:
    ...
    args.test_dir = folderPath
    args.test_name = "hazy_align,clear_align"
    tag = folderName
elif args.type == 6:
    ...
    predict_dir = os.path.join(predict_result)
    ...
    with torch.no_grad():
        ...
        for hazy, clean, name in tqdm(test_loader):
            ...
            if hazy.shape[1] != 3:
                os.system(
                    "rm -rf {}{}/{}".format(
                        args.test_dir, "hazy", name[0].split(".")[0] + ".jpeg"
                    )
                )
                continue
...
predict_dir = predict_result
if not os.path.exists(predict_dir):
    os.makedirs(predict_dir)

test_dataset = dehaze_test_dataset(args.test_dir, args.test_name, tag=tag, resize=False)
test_loader = DataLoader(
    dataset=test_dataset, batch_size=1, shuffle=False, num_workers=0, pin_memory=True
)
...
with torch.no_grad():
    ...
    for hazy, clean, name in test_loader:
        ...
        imwrite(frame_out, os.path.join(predict_dir, name[0]), range=(0, 1))
        print(os.path.join(predict_dir, name[0]))
