...
parser.add_argument(
    "-learning_rate", help="Set the learning rate", default=1e-4, type=float
)
parser.add_argument(
    "-train_batch_size", help="Set the training batch size", default=16, type=int
)
parser.add_argument(
    "-train_epoch", help="Set the training epoch", default=1000, type=int
)
parser.add_argument("--type", default=5, type=int, help="choose a type 012345")
...
parser.add_argument("--train_name", type=str, default="hazy,clear")
parser.add_argument(
    "--test_dir",
    type=str,
    default="",
)
parser.add_argument("--test_name", type=str, default="hazy,clear")
...
parser.add_argument(
    "-test_batch_size", help="Set the testing batch size", default=1, type=int
)
parser.add_argument(
    "--vgg_model", default="", type=str, help="load trained model or not"
)
...
if args.type == 0:
    ...
    args.train_dir = "/home/lx/nas/Dataset/03_RWIDE/05_other_datasets/NH-HAZE/train"
    args.train_name = "hazy,clear"
    args.test_dir = "/home/lx/nas/Dataset/03_RWIDE/05_other_datasets/NH-HAZE/test"
    args.test_name = "hazy,clear"
    tag = "outdoor"
    args.model_save_dir = "./output_result/NH-HAZE"
...
device = torch.device(
    "cuda:{}".format(device_ids[0]) if torch.cuda.is_available() else "cpu"
)
...
if args.restart:
    ...
    SDN.load_state_dict(
        torch.load(
            os.path.join(args.model_save_dir, name),
...
for epoch in range(start_epoch, train_epoch):
    ...
    with tqdm(total=len(train_loader)) as t:
        for hazy, clean in train_loader:
            ...
            total_loss = (
                ...
                + 0.0005 * adversarial_loss
                + 0.5 * msssim_loss_
                + 1.5 * smooth_loss_l1_img
                + 1.5 * (smooth_loss_l1_co + 0.01 * perceptual_loss_co + 0.5 * msssim_loss_co)
            )  # 0.01 * perceptual_loss_img + 0.0005 * adversarial_loss_img + 0.5 * msssim_loss_img +
            ...
            writer.add_scalars(
                "training", {"training total loss": total_loss.item()}, iteration
            )
            writer.add_scalars(
                "training_img",
                {
                    "img loss_l1": smooth_loss_l1.item(),
            ...
            co_loss = 1.5 * (
                smooth_loss_l1_co + 0.01 * perceptual_loss_co + 0.5 * msssim_loss_co
            )
            t.set_description(
    ...
    if args.seps:
        torch.save(
            SDN.state_dict(),
            os.path.join(args.model_save_dir, "epoch_" + str(epoch) + "_" + ".pkl"),
        )
        continue
    ...
    if epoch % sep == 0:
        ...
        with torch.no_grad():
            ...
            for hazy, clean, _ in tqdm(test_loader):
                ...
                hazy, ori_left, ori_right, ori_top, ori_down = padding_image(
                    hazy, max_h, max_w
                )
            ...
            if best_epoch_psnr == best_epoch_ssim:
                print(
                    "best epoch is {}, psnr: {}, ssim: {}".format(
                        best_epoch_psnr, best_psnr, best_ssim
                    )
                )
            else:
                print(
                    "best psnr epoch is {}: PSNR: {}, ssim: {}".format(
                        best_epoch_psnr, best_psnr, best_psnr_ssim
                    )
                )
                print(
                    "best ssim epoch is {}: psnr: {}, SSIM: {}".format(
                        best_epoch_ssim, best_ssim_psnr, best_ssim
                    )
                )
            print()
            ...
            writer.add_scalars(
                "testing", {"testing psnr": avr_psnr, "testing ssim": avr_ssim}, epoch
            )
            if best_epoch_psnr == epoch or best_epoch_ssim == epoch:
                torch.save(
                    ...
                    os.path.join(
                        ...
                        + str(epoch)
                        + "_"
                        + str(round(avr_psnr, 2))
                        + "_"
                        + str(round(avr_ssim, 3))
                        + "_"
                        + str(tag)
                        + ".pkl",
                    ),
